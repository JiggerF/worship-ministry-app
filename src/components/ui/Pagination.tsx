...
function Pagination({ page = 1, total = 1, onPrev, onNext, className, ...props }: { page?: number; total?: number; onPrev?: () => void; onNext?: () => void; className?: string } & React.ComponentProps<"nav">) {
  return (
    <nav
      ...
      className={cn("mx-auto flex items-center gap-3 text-gray-800", className)}
      {...props}
    >
      <button
        onClick={onPrev}
        disabled={page <= 1}
        className="px-3 py-1 border border-gray-300 rounded bg-white text-gray-800 hover:bg-gray-50 disabled:opacity-50"
      >
        Previous
      </button>
